...
Least-squares regression, logistic regression as inspiration, or elementary units of the graph.
...
Objective: learn the function $f(\cdot)$, such that $y=f_{\overline{W}}(\overline{X})$.
...
![width=10cm](img/perceptron.png)

## Perceptron
We choose the basic form of the function, but strive to find some parameters.

`Sign` is an activation function, value of the node is also sometimes referred to as an `activation`.
...
![width=10cm](img/linear_separation.png)
...
:::{.callout-important icon=false}
## Problems
In linearly separable data sets, a nonzero weight vector $W$ exists in which the sign
of $\overline{W}^T X$ correctly matches the sign of $y_i$ for each and every training instance $(\overline{X}_i,y_i)$.
...
Original perceptron did not formally use a loss function. *Retrospectively* we can introduce it as:
$$
...
\end{align*}
where $\Phi$ denotes \textit{activation function}.  
...
## tanh
$$
\Phi(v) = \max\left\{v, 0\right\}, \text{Rectified Linear Unit (ReLU)}
$$
:::
...
## Hard tanh
$$
\Phi(v) = \max\left\{\min\left[v, 1\right], -1 \right\}, \text{hard tanh}
$$
:::

## Multiple activation fns

![width=12cm](img/different_activation_functions.png)
...
Then column representations of these layers, denoted by $\overline{h}_1, \dots, \overline{h}_k$, have \textit{dimensionalities} $p_1, \dots, p_k$.
...
![width=12cm](img/multi_layer.png)
...
can be controlled very closely by the choice of parameters. The notion of learn-
ing refers to the setting of the parameters to make the overall function consistent
...
Let $\overline{x}$ be the $d$-dimensional
column vector corresponding to the input, $\overline{h_1},\dots,\overline{h_k}$ be the column vectors corresponding to the hidden layers, and $\overline{o}$ be the $m$-dimensional column vector corresponding to the output.
...
Note that one can replace the matrix $W_{k+1}W_k \dots W_1$ with the new $d\times m$ matrix $W_{xo}$, and
learn the coefficients of $W_{xo}$ instead of those of all the matrices $W1, W2, \dots W_{k+1}$, without
...
a single-layer network in terms of expressivity. $\qed$
...
a variable. Edges might be associated with learnable parameters. A variable in a node is
...
So we will have $\left\{(x^{(1)},y^{(1)}), ...(x^{(m)}, y^{(m)})\right\}$.
...
## Logistic Regression
$X \in \mathbb{R}^{n_x,m}$.
Using Numpy syntax: `X.shape = (n_x,m)`.

$Y \in \mathbb{R}^{1,m}$.
Using Numpy syntax: `Y.shape = (1,m)`.
...
\end{align*}
We have to define a \textit{loss (error) function} - this will estimate our model. 
...
![width=12cm](lec1/img/gradient-descent-2d-diagram.png)
...
\mybox{$(x_1,x_2,w_1,w_2,b)$} $\rightarrow$ \mybox{$z =w_1 x_1+w_2 x_2 + b$} $ \rightarrow $ \mybox{$\hat{y}=a=\sigma(z)$} $\rightarrow$ \mybox{$L(a,y)$}.
<!-- TODO tikz graph -->
...
\begin{frame}{Vectorizing logistic regression}
Let's examine the forward propagation step of LR.
\begin{align*}
  &z^{(1)} = w^T x^{(1)} + b,\\
  &a^{(1)} = \sigma(z^{(1)})
...
\begin{frame}[fragile]{Vectorizing logistic regression}
Next
$$
Z = [z^{(1)}, \dots, z^{(m)}] = w^T X + [b, b, \dots, b] = [w^T x^{(1)}+b, \dots, w^T x^{(m)}+b].
$$
