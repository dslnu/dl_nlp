...
While Feedforward Networks pass information through the network without cycles, the This enables them to extend the functionality of Feedforward Networks to also take into account previous inputs $\bb{X}_{0:t-1}$ and not only the current input $\bb{X}_t$. 
:::
...
:::{.callout-tip icon=false}
## Language modeling
We aim to predict the next token based on the current and past tokens; thus we shift the original sequence by one token as the targets (labels). Bengio et al. (2003) first proposed to use a neural network for language modeling.
:::
...
In practice, each token is represented by a $d$-dimensional vector, and we use a batch size . Therefore, the input $\bb{X}_t$ at time step $t$ will be an $n \times d$ matrix.
